...
export interface AssembleContextInput {
  ...
  /** Visibility-scoped cross-session root index to prepend as protected context. */
  rootSummary?: string;
}
...
export class ContextAssembler {
  ...
  async assemble(input: AssembleContextInput): Promise<AssembleContextResult> {
    ...
    const rootSummaryPayload = input.rootSummary?.trim()
      ? `<root_index>\n${input.rootSummary.trim()}\n</root_index>`
      : undefined;
    const rootSummaryTokens = rootSummaryPayload ? estimateTokens(rootSummaryPayload) : 0;
    ...
    const remainingBudget = Math.max(0, tokenBudget - rootSummaryTokens - tailTokens);
    ...
    const estimatedTokens = rootSummaryTokens + evictableTokens + tailTokens;
    ...
    const rootSummaryMessage = rootSummaryPayload
      ? ({ role: "user", content: rootSummaryPayload } as AgentMessage)
      : undefined;
    const cleanedWithRootSummary = rootSummaryMessage ? [rootSummaryMessage, ...cleaned] : cleaned;
    ...
    const preSanitizeFreshTailMessages = cleanedEntries
      ...
      .map((entry) => entry.message);
    const repaired = sanitizeToolUseResultPairing(cleanedWithRootSummary) as AgentMessage[];
    return {
      messages: repaired,
      estimatedTokens,
      stats: {
      ...
      debug: {
        ...
        preSanitizeMessagesHash: hashMessages(cleanedWithRootSummary as AgentMessage[]),
